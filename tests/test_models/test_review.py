...
class TestReview(unittest.TestCase):
    ...
    def test_unique_id_per_instance(self):
        """Test that each Review instance has a unique id. Because every opinion is special."""
        review2 = Review()
        self.assertNotEqual(self.review.id, review2.id)

    def test_id_is_string(self):
        """Test that id attribute is a string. A review without an ID is like a book without a cover."""
        self.assertIsInstance(self.review.id, str)

    def test_datetime_attributes(self):
        """Test that created_at and updated_at are datetime objects. Time ticks on, even for reviews."""
        self.assertIsInstance(self.review.created_at, datetime)
        self.assertIsInstance(self.review.updated_at, datetime)

    def test_different_created_at_for_multiple_instances(self):
        """Test different created_at times for distinct instances. Every review has its moment."""
        review2 = Review()
        sleep(0.01)
        review3 = Review()
    ...
    def test_public_attributes(self):
        """Test that Review class has expected public attributes. Nothing but the truth here."""
        self.assertEqual(self.review.place_id, "place_1234")
        self.assertEqual(self.review.user_id, "user_5678")
        self.assertEqual(self.review.text, "Amazing place! Just mind the raccoons.")
    ...
    def test_save_creates_file(self):
        """Test that save() creates file.json. The review is saved, even if the experience is questionable."""
        self.review.save()
        self.assertTrue(os.path.exists("file.json"))

    def test_save_file_content(self):
        """Test that save() writes correct data to file.json. Raccoons included."""
        self.review.save()
        with open("file.json", "r", encoding="utf-8") as f:
            data = json.load(f)
        key = f"Review.{self.review.id}"
        self.assertIn(key, data)
        self.assertEqual(data[key]["text"], "Amazing place! Just mind the raccoons.")
    ...
    def test_to_dict_includes_all_attributes(self):
        """Test that to_dict() includes all Review attributes. Every detail matters."""
        ...
        self.assertEqual(review_dict["text"], "Amazing place! Just mind the raccoons.")

    def test_to_dict_datetime_format(self):
        """Test that to_dict() converts datetime attributes to ISO format. Time, but make it stringy."""
        review_dict = self.review.to_dict()
    ...
    def test_to_dict_additional_attributes(self):
        """Test that to_dict() includes dynamically added attributes. All feedback counts."""
    ...
    def test_to_dict_with_invalid_argument(self):
        """Test that to_dict() with invalid arguments raises TypeError. This method is VIP-only."""
        with self.assertRaises(TypeError):
            self.review.to_dict(None)

    def test_to_dict_output(self):
        """Test that to_dict() output matches expected dictionary. All the details, down to the raccoons."""
